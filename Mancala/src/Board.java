...
public class Board{
	int numHouses;
	public int [][] board;
	static int [] indexToRow;
	static int [] indexToCol;
	...
	int [] randomArray;
	public boolean pieMoveConsidered;
	int numSeeds;
	
	/** Initializes a board for use in the client/server.  
	 * 	
	 * @param _numHouses The number of houses.  Can be from 4-9.
	 * @param _numSeeds Can be from 4-9.
	 * @param _randomSeeds True if you want to randomly distribute the seeds.
	 */
	...
	public Board(int _numHouses, int _numSeeds,boolean _randomSeeds, int[] randomSeeds){
		numHouses = _numHouses;
		numSeeds = _numSeeds;
		board = new int [2][numHouses];
		initializeIndexMappings(numHouses);
		...
		for(int i = 0; i < numHouses; i ++){
			board[0][i] = _randomSeeds ?  randomSeeds[numHouses-1-i] : _numSeeds;
			board[1][i] = _randomSeeds ? randomSeeds[i] : _numSeeds;
		}
	 ...
	 * 
	 * @return Randomly distributed array.
	 */
	...
	int [] createRandomArray(int numHouses){
		randomArray = new int[numHouses];
		int counter = 0;
		while(true){
			while(counter < numHouses*numSeeds){
	 ...
	 * Used in the clone and test functions.
	 ...
	 * The children are the possible boards that can be made in one player's turn including the pie move.
	...
	public ArrayList<Board> getChildren(boolean playerOneOrTwo){
		...
		if(this.moveCounter == 1 && !pieMoveConsidered){
			Board pieChild = cloneChild(this);
			pieChild.movePiece(-1, -1, true);
			pieMoveConsidered = true;
			boards.add(pieChild);
		}
	...
	public Board cloneChild(Board toCopy){
		...
		child.pieMoveConsidered = toCopy.pieMoveConsidered;
	...
	/** Copies the board.
	 * 
	 * @param toCopy board you want to copy.
	 * @return the copied board
	 */
	public Board copy(Board toCopy){
		Board copied = new Board(toCopy.board, toCopy.storePlayer1, toCopy.storePlayer2, toCopy.numHouses);
		copied.pieMoveConsidered = toCopy.pieMoveConsidered;
		copied.moveCounter = toCopy.moveCounter;
		copied.parent = toCopy.parent;
		copied.previous = toCopy.previous;
		copied.pieMoveConsidered = toCopy.pieMoveConsidered;
		copied.pitsOnSide1 = toCopy.pitsOnSide1;
		copied.pitsOnSide2 = toCopy.pitsOnSide2;
		return copied;
	}
	...
	public boolean movePiece(int _row, int _column, boolean _pieRule){
		...
		if((_row == 0 || _row == 1) && (_column >= 0 && _column < numHouses)){ 
			...
			if(piecesToMove == 0){
				throw new IllegalArgumentException("Invalid move" + _row + " " + _column);
			}
	 ...
	 * Used in the getChildren method. 
	 ...
	 * 
	 * @param row The row of the move you want to make.
	 * @param column The column of the move you want to make.
	 * @return true if it's valid and false if it is.
	 */
